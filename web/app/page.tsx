...
export default function Page() {
  ...
  async function loadAt(lat: number, lng: number) {
    ...
    try {
      ...
      if (json.error) {
        setError(`API Error: ${json.error}${json.details ? ` - ${json.details}` : ''}`);
        setObservations([]);
        return;
      }
      ...
      if (json.meta?.photoStats) {
        console.log('Photo stats:', json.meta.photoStats);
      }
      ...
      <div className="flex gap-6 p-6">
        ...
        {showMap && (
          <div className="h-96 w-1/2 rounded-lg overflow-hidden shadow-md border border-gray-200 flex-shrink-0">
            <MapView latitude={displayLat!} longitude={displayLng!} />
          </div>
        )}

        {/* Birds List Section */}
        <div className={`${showMap ? "w-1/2" : "flex-1"} flex flex-col bg-white rounded-lg shadow-md border border-gray-200 p-6`}>
          <h2 className="text-2xl font-bold mb-4">Bird Sightings</h2>

          {geoLoading && <p className="text-gray-500">Getting your location...</p>}
          {loading && <p className="text-gray-500">Loading birds...</p>}
